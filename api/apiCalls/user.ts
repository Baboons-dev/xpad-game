...
export const userLoginWallet = async (payload: {
  wallet_address: string;challenge:string,signature:string
}): Promise<any> => {
  try {
...
export const sendFund = async (client: string,hash:string,chainId:string | number,amount:string|number): Promise<any> => {
  try {
    const endPoint = '/api/funds/'+client+'/';
    const res = await axios.post<any>(endPoint, {hash:hash,chainId:chainId,amount:amount});
    if (!res?.data) throw 'Something went wrong GetUser';
    // console.log('GetUser>>', res.data);
    return res;
  } catch (err) {
